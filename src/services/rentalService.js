...
export const subscribeToRentalsByRole = (userId, role, callback) => {
  ...
  if (role === 'ADMIN') {
    ...
    q = query(collection(db, RENTALS_COLLECTION), where('managerId', '==', userId), orderBy('date', 'desc'));
  } else if (role === 'OWNER') {
...
// GÜNCELLENMİŞ CREATE: Komisyon Mantığı
export const createRental = async (rentalsDataArray, userRole, managerId = null, commissionRate = 0) => {
  ...
  rentalsDataArray.forEach(rental => {
    ...
    if (userRole === 'MARKET_MANAGER' && managerId) {
        ...
        rentalData.ownerRevenue = rental.price - commission; // Sahibin Net Payı
    }
    ...
    if (item.isGroup) {
      ...
      if (amount > 0) { 
        ...
        for (const rental of rentals) {
          ...
          batch.update(rental.ref, { paidAmount: isFullyPaid ? price : newPaid, isPaid: isFullyPaid });
          remaining -= pay;
        }
      } else { 
      ...
      batch.update(ref, { paidAmount: isFullyPaid ? price : newPaidAmount, isPaid: isFullyPaid });
    }
    await batch.commit();
